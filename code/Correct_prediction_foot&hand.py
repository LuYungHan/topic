...
def RF(data,data1):
    ...
    data = np.array(pd.read_csv(data))
    data_len = len(data[:,1])
    labelSeries = data[1:data_len-16,31]
    dataSeries = np.append(data[1:data_len-16,1:4], data[1:data_len-16,7:10] , axis=1)
    dataSeries = np.append(dataSeries, data[1:data_len-16,16:19] , axis=1)
    dataSeries = np.append(dataSeries, data[1:data_len-16,22:25] , axis=1)
    data1 = np.array(pd.read_csv(data1))
    data_len1 = len(data1[:,1])
    labelSeries1 = data1[1:data_len1-16,31]
    dataSeries1 = np.append(data1[1:data_len1-16,1:4], data1[1:data_len1-16,7:10] , axis=1)
    dataSeries1 = np.append(dataSeries1, data1[1:data_len1-16,16:19] , axis=1)
    dataSeries1 = np.append(dataSeries1, data1[1:data_len1-16,22:25] , axis=1)
